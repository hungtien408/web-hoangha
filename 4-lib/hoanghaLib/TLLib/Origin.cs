...
namespace TLLib
{
    public class Origin
    {
        ...
        public DataTable OriginSelectAllByProductCategoryID(
        ...
        )
        {
            try
            {
                var dt = new DataTable();
                var scon = new SqlConnection(connectionString);
                var cmd = new SqlCommand("usp_Origin_SelectAllByProductCategoryID", scon);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                if (errorCodeParam.Value.ToString() != "0")
                    throw new Exception("Stored Procedure 'usp_Origin_SelectAllByProductCategoryID' reported the ErrorCode : " + errorCodeParam.Value.ToString());
